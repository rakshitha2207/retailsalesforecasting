...
with tab1:
    ...
    # Create the main line plot for the actual sales data
    fig = px.line(
        entire_data, 
        x=entire_data.index, 
        y=entire_data, 
        title='<b>Real Advance Retail Sales in US: Retail Trade</b>'
    )
    ...
    fig.update_xaxes(
        ...
        showline=True, linewidth=1, linecolor='gray', gridcolor='lightgray'
    )
    fig.update_yaxes(
        title_text='<b>Billions of 2023 Dollars</b>', 
        tickfont=dict(family='Arial', size=12),
        showline=True, linewidth=1, linecolor='gray', gridcolor='lightgray'
    )

    # Customize layout for title and background color
    fig.update_layout(
        title=dict(x=0.4, y=0.95, font=dict(size=22, color='darkblue')),
        plot_bgcolor='rgba(245, 245, 250, 0.9)',  # Light shaded background
        legend=dict(font=dict(size=13), orientation="h", yanchor="top", y=1.15, xanchor="center", x=0.5)
    )

    # Add Forecast trace with dashed line for distinction
    ...
    # Add Actual values with star markers
    fig.add_trace(go.Scatter(
        x=test["2023-05-01":].index,
        y=test["2023-05-01":],
        mode='markers',
        marker=dict(symbol='star', color='purple', size=10, line=dict(color='purple', width=1)),  # Updated line color
        name='<b>Actual</b>',
        hovertemplate='Month: %{x}<br>Sales: %{y:.2f}<extra></extra>'
    ))

    # Add Upper and Lower 95% Confidence Interval with shaded fill and smooth line
    fig.add_trace(go.Scatter(
    ...
    # # Update hover mode and overall layout for better readability and professional look
    # fig.update_layout(
    #     hovermode="x unified",
    ...
    fig.update_layout(height=600, width=1200, showlegend=True, margin=dict(t=160, b=0, l=0, r=0))
    ...
    min_length = min(len(test), len(hw_forecast_dev_mean))
    aligned_test = test[:min_length]
    aligned_forecast = hw_forecast_dev_mean[:min_length]
...
# Use streamlit.dataframe to display the styled DataFrame
